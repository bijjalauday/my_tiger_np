...
from src.pipeline import efficientnet, matcher
...
def enroll_pet(
    ...
    for pet_id in house_df["pet_id"].unique():
        ...
        pred_pet_valid_faces = list(filter(lambda x: x is not None, pet_imgs))
...
def infer_pet_id(
    ...
    house_df = house_df[house_df["house_id"] == house_id]
    # if dog images present --> show notification
    if "dog" in house_df["category"].values:
        pred_dogs = house_df[house_df["category"] == "dog"]["image_path"].values
        print(f"{len(pred_dogs)} dog images detected and ignoring for inference.")
        house_df = house_df[house_df["category"] == "cat"]
        pet_imgs_paths = house_df["image_path"].values
        img_names = [im.split("\\")[-1][:-4] for im in pet_imgs_paths]
    else:
        pet_imgs_paths = house_df["image_path"].values
        img_names = [im.split("\\")[-1][:-4] for im in pet_imgs_paths]
    # print(img_names)
    ...
    # print(len(PET_IMGS),house_id)

    # read top_frames.xlsx file
    excel_path = os.path.join(infer_dir_path, house_id, "top_predictions.xlsx")
    ...
    for imgs in img_names:
        pred_pet_types.append(
            pred_exl[pred_exl["image_name"] == imgs]["category"].values[0]
        )
        pred_pet_coords.append(
            pred_exl[pred_exl["image_name"] == imgs]["bbox_coordinates"].values[0]
        )
        pred_conf_scores.append(
            pred_exl[pred_exl["image_name"] == imgs]["confidence_score"].values[0]
        )
    ...
    # print(enrolled_hh_pets_db["pet_id"],house_id)
    pred_pet_ids, proba, res_orig = matcher.get_pet_id(
        pred_pet_embds, pred_pet_types, enrolled_hh_pets_db
    )
    house_df["pred_pet_type"] = pred_pet_types
    house_df["pred_pet_id"] = pred_pet_ids
    house_df["proba"] = proba
    house_df["res_orig"] = res_orig
