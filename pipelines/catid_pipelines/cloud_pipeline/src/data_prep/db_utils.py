...
def load_enrolled_pet_db(context):
    db_path = context.data_catalog["enroll"]["enrolled_pet_db"].format(
        **context.data_catalog["enroll"]
    )
...
def load_enroll_metadata_db(context):
    db_path = context.data_catalog["enroll"]["raw_metadata_db"].format(
        **context.data_catalog["enroll"]
    )
    if os.path.exists(db_path):
        db_csv = _load_db(db_path)
...
def load_inference_label_db(context):
    db_path = context.data_catalog["inference"]["inference_truth_label_db"].format(
        **context.data_catalog["inference"]
    )
    if os.path.exists(db_path):
...
def save_inference_metadata_db(context, data):
    db_path = context.data_catalog["inference"]["raw_metadata_db"].format(
        **context.data_catalog["inference"]
    )
    return _save_db(data, db_path)
...
def save_inference_prediction_db(context, data):
    db_path = context.data_catalog["inference"]["inference_preds_db"].format(
        **context.data_catalog["inference"]
    )
    return _save_db(data, db_path)
...
def _training_model_version(context, model):
    if model == "efficientnet":
        curr_versions = os.listdir(
            context.data_catalog["efficientnet"]["artifacts_folder"]
        )
    elif model == "yolo":
...
def load_efficientnet_train_metadata_db(context):
    db_path = context.data_catalog["efficientnet"]["raw_metadata_db"].format(
        **context.data_catalog["efficientnet"]
    )
    if os.path.exists(db_path):
        db_csv = _load_db(db_path)
    else:
...
def load_efficientnet_processed_train_metadata_db(context):
    db_path = context.data_catalog["efficientnet"]["model_metadata_db"].format(
        context.data_catalog["efficientnet"]
    )
    if os.path.exists(db_path):
        db_csv = _load_db(db_path)
...
def save_efficientnet_processed_train_metadata_db(context, data, training_verison):
    ...
    db_path = os.path.join(
        os.path.dirname(db_path), training_verison, "train", os.path.basename(db_path)
    )
    return _save_db(data, db_path)
...
def load_yolo_train_metadata_db(context):
    ...
    if os.path.exists(db_path):
        ...
        db_csv = pd.DataFrame(
            columns=["datetime", "house_id", "pet_type", "pet_id", "image_path"]
        )
    return db_csv


def save_yolo_train_metadata_db(context, data):
    db_path = context.data_catalog["yolo"]["raw_metadata_db"].format(
        **context.data_catalog["yolo"]
    )
    return _save_db(data, db_path)


def load_yolo_processed_train_metadata_db(context):
    db_path = context.data_catalog["yolo"]["model_metadata_db"].format(
        **context.data_catalog["yolo"]
    )
    if os.path.exists(db_path):
        db_csv = _load_db(db_path)
...
def write_pipeline_report(report, report_path=""):
    ...
    yolo_cls_report_df.to_excel(
        writer,
        startrow=detect_count_df.shape[0] + 3,
        index=True,
        sheet_name=f"pets_type_report",
    )
    yolo_cfm_matrix_df.to_excel(
        writer,
        startrow=detect_count_df.shape[0] + 3,
        startcol=yolo_cls_report_df.shape[1] + 2,
        index=True,
        sheet_name=f"pets_type_report",
    )
