...
def prep_enroll_data(context, enrolled_pets=None):
    """
        ...
        for pet_type in os.listdir(os.path.join(dir_path, house_name)):
            for pet in os.listdir(os.path.join(dir_path, house_name, pet_type)):
                ...
                if house_pet_id in enrolled_pets:
                    continue
                for pet_image in os.listdir(os.path.join(dir_path, house_name, pet_type, pet)):
                    img_path = os.path.join(house_name, pet_type, pet, pet_image)
                    img = cv2.imread(os.path.join(dir_path, img_path))
                    if img is None:
                        continue
                    image_info.append([_datetime.now().strftime("%d-%m-%Y %H:%M:%S"),
                                       house_name, pet, house_pet_id, pet_type, img_path])

    meta_df_cols = ['datetime', 'house_id', 'pet_id', 'house_pet_id', 'pet_type', 'image_path']
...
def prep_inference_data(context):
    ...
    for house_name in _tqdm(os.listdir(dir_path)):
        if not os.path.isdir(os.path.join(dir_path, house_name)):
            continue
        for pet_image in os.listdir(os.path.join(dir_path, house_name)):
            img_path = os.path.join(house_name, pet_image)
            img = cv2.imread(os.path.join(dir_path,img_path))
            if img is None:
                continue
            image_info.append(
                [_datetime.now().strftime("%d-%m-%Y %H:%M:%S"), house_name, img_path])

    df_new = pd.DataFrame(image_info, columns=['datetime', 'house_id', 'image_path'])
    if len(inference_labels_df) > 0:
        df_new = df_new.merge(inference_labels_df, how='left', on=['house_id', 'image_path'])
    else:
    ...
    for house_name in _tqdm(os.listdir(dir_path)):
        ...
        for pet_type in os.listdir(os.path.join(dir_path, house_name)):
            for pet in os.listdir(os.path.join(dir_path, house_name, pet_type)):
                ...
                house_pet_id = f"{house_name}-{pet}"
                for pet_image in os.listdir(os.path.join(dir_path, house_name, pet_type, pet)):
                    img_path = os.path.join(dir_path, house_name, pet_type, pet, pet_image)
                    img = cv2.imread(img_path)
                    if img is None:
                        continue
                    image_info.append(
                        [_datetime.now().strftime("%d-%m-%Y %H:%M:%S"), house_name, pet, house_pet_id,
                         pet_type, os.path.join(house_name, pet_type, pet, pet_image)])
    ...
    for img in os.listdir(os.path.join(dir_path, 'images')):
        ...
        if label_name_ in os.listdir(os.path.join(dir_path, 'labels')):
            if os.stat(os.path.join(dir_path, 'labels', label_name_)).st_size != 0:
                ...
                if count > 1:
                    pet_type = 'multiple'
                info.append([_datetime.now().strftime("%d-%m-%Y %H:%M:%S"), img, label_name_,
                             os.path.join('images', img), pet_type, 1])

            else:
                info.append([_datetime.now().strftime("%d-%m-%Y %H:%M:%S"), img, label_name_,
                             os.path.join('images', img), 'other', 1])
        else:
            info.append([_datetime.now().strftime("%d-%m-%Y %H:%M:%S"), img, np.nan,
                         os.path.join('images', img), np.nan, 0])

    meta_df_cols = ['datetime', 'image_name', 'label_name', 'image_path', 'pet_type', 'to_use']
    ...
    for root, dirs, files in os.walk(db_path):
        for img in files:
            ...
            if label_path_ in files:
                shutil.copy(os.path.join(root, img), os.path.join(out_imgs_path, out_img_name))
                shutil.copy(os.path.join(root, label_path_),
                            os.path.join(out_labels_path,  out_img_name+out_img_name.rsplit('.', 1)[-1]))
