...
def get_pet_id(pred_pet_embds, pred_pet_types, enrolled_hh_pets_db):
    ...
    for i in enrolled_hh_pets_db["embedding"].values:
        l.append(i)
    df=pd.DataFrame(l)     # taking all the embeddings and creating a dataframe from it 
    print(df.shape)
    df["pet_id"]=enrolled_hh_pets_db["pet_id"].values
    le=LabelEncoder()
    df["orig"]=le.fit_transform(df["pet_id"])   #the values columns is the each cat name. So, we are converting it to numrical values
    ...
    for pred_embed, pred_pet_type in zip(pred_pet_embds, pred_pet_types):
        if ch==1:
            ...
            thresh=thresh/100       #creating the threshold value based on the number of classes 
            pred_pet_id=lr.predict([pred_embed])[0]
            proba=list(lr.predict_proba([pred_embed])[0])  #predicting the outputs of each embedding 
            # print(proba,thresh,pred_pet_id)
            res1.append(proba[pred_pet_id])
            res_orig.append(le.inverse_transform([pred_pet_id])[0]) # from the outputs, performing inverse label encoding to get the cat names
